...
| ID | Layar | Isi |
|---|---|---|
| M1 | Jadwal | Bilah kendali menempel di atas: saringan jenis kelas, saringan pelatih, pemilih tanggal, sakelar rupa (DS-51) · dua rupa untuk data yang sama: kalender mingguan (bawaan, ≥ 768px, baris jam setinggi isinya — DS-52) dan daftar satu hari (`?rupa=daftar`, selalu dipakai di HP) · sisa kursi · di rupa daftar tiap baris yang bisa ditindaki punya tombolnya sendiri ("Pesan" / "Antre"), di kalender kartunya sendiri yang jadi tombol · kartu sesi membuka panel konfirmasi M2, kartu penuh langsung mengantre · di dalam aplikasi bilah kendali menyatu dengan kotak jadwal dan panel buat-kelas ikut menempel (DS-57) — `src/app/jadwal/`. **Terbuka untuk tamu**: pengunjung yang belum masuk melihat jadwal yang sama dalam kerangka publik, tanpa tombol booking dan tanpa kartu kredit (DS-45) |
...
| A6 | Kelas & paket | Tiga angka ringkas di atas (kelas · paket · paket terjual), lalu dua tabel bersebelahan — **paket 8 kolom, kelas 4** — masing-masing dengan cari dan halamannya sendiri (DS-56) · aksi per baris di menu "⋯", formulirnya dialog dari URL · kelas selalu bisa diubah, paket hanya selama belum dibeli siapa pun (DS-53) · peringatan **kursi vs kredit** muncul hanya saat kursi terjadwal kurang dari kredit yang sudah terjual (DS-55) — `src/app/admin/layanan/` |
