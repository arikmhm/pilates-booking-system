...
| Tanggal | Perubahan |
|---|---|
| 2026-09-23 | **Jadwal: bilah kendali menempel, kalender berbaris isi** (DS-51 direvisi, DS-52 baru). Saringan chip diganti dua `<select>` — jenis kelas dan **pelatih**, saringan baru — ditambah `<input type="date">` dan sakelar rupa, semuanya dalam satu bilah `sticky top-0`. Dua parameter waktu (`?minggu=` offset + `?hari=` indeks) dilebur jadi satu jangkar `?tgl=YYYY-MM-DD` yang menentukan minggu sekaligus hari; pemilih tanggal membuat salah satunya mustahil dipetakan tanpa yang lain. Strip hari berdiri sendiri dihapus: ia mengulang ketujuh tanggal yang sudah ada di kepala kolom kalender — sekarang tombol geser minggu menyatu di dua lajur tepi kepala kalender, dan kepala kolomnya berhenti jadi tautan. Komponen kepala yang sama dipakai lagi di rupa daftar, di sana dengan tautan karena cuma satu hari yang digambar. **Kalender berhenti memakai `position:absolute` setinggi durasi**: jam tanpa kelas tidak digambar, sesi berbarengan ditumpuk di petak yang sama alih-alih dibagi jadi lajur separuh kolom, tiap kartu setinggi isinya, dan durasi ditulis apa adanya ("08.00 · 70m"). Label jam pindah ke dalam barisnya, tidak lagi membelah garis. `kendali.tsx` jadi komponen klien pertama di layar ini — hanya karena `<select>` tidak bisa pindah halaman sendiri; formulirnya tetap `method="get"` dengan tombol kirim di `<noscript>` |
| 2026-09-23 | **Layar jadwal dirombak: strip hari + dua rupa** (DS-51). Rentang tanggal plus tiga tombol geser diganti strip tujuh hari — nama hari di atas nomor tanggal, `grid-cols-7`, diapit tombol minggu — jadi "Kamis ada apa" dijawab satu ketukan. Data yang sama sekarang punya dua rupa yang tersimpan di URL (`?rupa=daftar`): kalender menjawab bentuk minggunya (jeda, tumpukan, lubang), daftar menjawab isi satu hari dengan jam mulai di atas jam selesai. Di bawah 768px rupanya selalu daftar dan sakelarnya disembunyikan; tumpukan tujuh kartu hari versi HP dihapus karena stripnya sudah menjawab pertanyaan yang sama. **Kalender berhenti digulung tegak** — `max-h` dibuang, seluruh harinya digambar sekaligus, karena lubang jadwal justru yang hilang saat dipotong setinggi layar. Yang dibayar: baris hari tidak lagi menempel di atas, dan strip di atasnya yang menanggung tugas itu. Batang warna per jenis kelas dari referensinya sengaja tidak diambil — palet sistem ini memetakan warna ke status, bukan ke alat (DS-14) |
| 2026-09-23 | **Katalog paket `/paket`** — halaman publik ketiga, kerangka yang sama dengan jadwal (bilah atas bersama + hero pendek 160/208px). Bedanya dengan pita harga di halaman profil disengaja: pita itu brosur — tiga paket pilihan, statis, prerender; `/paket` katalog yang membaca `packages`, jadi paket baru dari A6 terbit tanpa menyentuh kode (DS-49). Tata letaknya dipimpin BR-1.4: jenis kelas yang tercakup jadi label di kepala kartu dan saringannya memakai jenis kelas — di seed, Mat cuma tercakup 2 dari 3 paket, dan itu langsung kelihatan. Slot foto diberi label "Foto menyusul" apa adanya (DS-50). Tombolnya WhatsApp meja depan dengan pesan yang sudah menyebut paketnya, karena pembelian mandiri baru versi real (UC-M11). Kerangka publik pindah ke `components/rangka-publik.tsx` begitu halaman kedua memakainya |
| 2026-09-23 | **Transaksi dan kredit dipisah tegas** (DS-48). Keduanya sama-sama "riwayat", tapi satuannya beda: transaksi dihitung rupiah dan satu baris satu pembayaran; kredit itu saldo dan barisnya gerak saldo. Detail transaksi berhenti memuat buku besar mentah — sekarang ia memuat apa yang dibeli (kredit diterbitkan, masa berlaku, **jenis kelas yang tercakup** BR-1.4) plus ringkasan empat angka beserta persamaannya, lalu menautkan ke buku kredit. Kolom "Rincian" di daftar dihapus: bahasa kredit di layar uang. A3 jadi satu-satunya layar yang sengaja memuat dua-duanya — di sana pertanyaannya tentang orangnya, bukan tentang satu pembayaran — dan dapat kartu daftar transaksi baru. Sekalian butir menu "Halaman Publik" dihapus dari semua peran |
| 2026-09-23 | Buku transaksi dapat **penomoran halaman** (25 per halaman, `limit`/`offset` + `count(*) over ()` di query yang sama) dan **halaman detail** `/transaksi/[id]`: buku besar satu paket baris per baris, ditutup baris Sisa yang harus sama dengan jumlah kolom kanannya (BR-1.7). Baris tanggal di tabel sekarang menuju transaksinya, bukan profil member — profilnya satu klik lagi dari sana. Kepemilikan dijaga di dalam query: member yang mengetik id paket orang lain dapat 404, coach dipantulkan ke daftar. Komponen `Halaman` dipakai bersama A4, yang salinan pager lokalnya dihapus (DS-36) |
| 2026-09-23 | **Layar Transaksi** — satu rute `/transaksi`, empat tampilan (UC-M14, UC-A17, UC-O09, UC-C03). Member melihat tiap paket yang dibelinya beserta nasib kreditnya (jadi kelas · kembali · hangus), coach melihat kredit yang terpakai di kelas yang ia ajar tanpa satu pun angka rupiah, admin melihat buku transaksi studio plus kredit yang dipindah tangan lewat koreksi manual, dan pemilik melihat buku yang sama **plus** omzet, rata-rata, dan nilai kredit hangus (BR-9.3). Tanpa tabel baru: `payments` masih fase dua (BR-8.1–8.2), jadi yang dibaca `member_packages` + `credit_ledger`. Dua test integrasi, dibuktikan lewat mutasi — kursi yang batal tepat waktu sempat terhitung memakan kredit karena query cuma membaca baris `booking` dan melewatkan baris balasannya |
| 2026-09-23 | Sidebar staf dikelompokkan ulang menurut "saya mau mengurus apa": Dashboard berdiri sendiri, lalu **Jadwal** (Jadwal Kelas · Aturan Jadwal), **Member** (Direktori Member · Pesan Terkirim), **Studio** (Pelatih & Staf · Layanan & Paket · Halaman Publik), dan **Bisnis** (Laporan, hanya owner — BR-9.3). Sebelumnya sembilan butir dalam dua kelompok yang menamai waktu, bukan urusan. Akordeon sidebar-07 shadcn sempat dipasang lalu dilepas lagi: tiap kelompok cuma dua sampai tiga butir dan semuanya terbuka sejak awal, jadi yang ditambahkan hanya tombol untuk menyembunyikan isi sidebar plus satu komponen klien di kerangka yang selebihnya dirender server (DS-33) |
